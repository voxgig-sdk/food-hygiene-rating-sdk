...
// FoodHygieneRating SDK feature test
//
...
class FtCtrl extends FoodHygieneRatingControl
{
...
class FtClient
{
    ...
    private FoodHygieneRatingUtility $utility;

    public function __construct(FoodHygieneRatingUtility $utility, array $options)
    {
    ...
    public function get_utility(): FoodHygieneRatingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FoodHygieneRatingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FoodHygieneRatingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FoodHygieneRatingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FoodHygieneRatingConfig::make_feature($name);
    ...
    private static function buildUrl(FoodHygieneRatingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FoodHygieneRatingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FoodHygieneRatingResponse($fetched);
        }
    ...
    private function fail(FoodHygieneRatingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FoodHygieneRatingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
