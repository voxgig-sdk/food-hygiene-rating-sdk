// Command FoodHygieneRating-cli is a CLI + REPL for the FoodHygieneRating SDK,
...
const prompt = "food-hygiene-rating"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "authority business_type establishment rating"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FOOD_HYGIENE_RATING_APIKEY (api key), FOOD_HYGIENE_RATING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FOOD_HYGIENE_RATING_APIKEY carries the API key and
	// FOOD_HYGIENE_RATING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FOOD_HYGIENE_RATING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FOOD_HYGIENE_RATING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFoodHygieneRatingSDK(opts)
