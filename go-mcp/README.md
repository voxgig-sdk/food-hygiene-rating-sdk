# food-hygiene-rating-mcp

[MCP](https://modelcontextprotocol.io) server exposing the FoodHygieneRating SDK as
two agent tools — `food-hygiene-rating_list` and `food-hygiene-rating_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/food-hygiene-rating-mcp)
make build
...
claude mcp add --scope user food-hygiene-rating \
  -- /absolute/path/to/food-hygiene-rating-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./food-hygiene-rating-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// food-hygiene-rating_list: first page of records
{ "entity": "authority" }
{ "entity": "authority", "query": { } }

// food-hygiene-rating_load: one record by id
{ "entity": "authority", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/food-hygiene-rating-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user food-hygiene-rating \
     -- "$PWD"/dist/*/food-hygiene-rating-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `food-hygiene-rating_list` and `food-hygiene-rating_load` tools now appear
   in new sessions. Ask the agent to *"list authority using food-hygiene-rating"*
   and it calls `food-hygiene-rating_list` with `{"entity":"authority"}`.
...
### Call the `food-hygiene-rating_list` tool
...
### Call the `food-hygiene-rating_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `food-hygiene-rating_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `food-hygiene-rating_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./food-hygiene-rating-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"food-hygiene-rating_load","arguments":{"entity":"authority","query":{"id":1}}}}'
```
