...
type Args struct {
	Entity string         `json:"entity" jsonschema:"authority | business_type | establishment | rating"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.FoodHygieneRatingSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "food-hygiene-rating_list",
		Description: "List records from FoodHygieneRating. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "food-hygiene-rating_load",
		Description: "Load a single record from FoodHygieneRating. " +
...
func runOp(client *sdk.FoodHygieneRatingSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.FoodHygieneRatingSDK, name string) (sdk.FoodHygieneRatingEntity, error) {
	switch strings.ToLower(name) {
	case "authority":
		return client.Authority(nil), nil
	case "business_type":
		return client.BusinessType(nil), nil
	case "establishment":
		return client.Establishment(nil), nil
	case "rating":
