...
func TestBusinessTypeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.BusinessType(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.BusinessType(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		businessTypeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.business_type")))
...
func business_typeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FOOD_HYGIENE_RATING_TEST_BUSINESS_TYPE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FOOD_HYGIENE_RATING_TEST_BUSINESS_TYPE_ENTID"])
