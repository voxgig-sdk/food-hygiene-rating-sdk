...
import (
	...
	omni "github.com/voxgig-sdk/food-hygiene-rating-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FoodHygieneRatingError); is {
				out := map[string]any{
					"name":    "FoodHygieneRatingError",
...
func omniCtx(arg any, client *sdk.FoodHygieneRatingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FoodHygieneRatingSDK); is {
			client = live
