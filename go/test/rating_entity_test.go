...
func TestRatingEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Rating(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Rating(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FOOD_HYGIENE_RATING_TEST_RATING_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		ratingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.rating", setup.data)))
		var ratingRef01Data map[string]any
		if len(ratingRef01DataRaw) > 0 {
			ratingRef01Data = core.ToMapAny(ratingRef01DataRaw[0][1])
		}
		...
		ratingRef01Match := map[string]any{}

		ratingRef01ListResult, err := ratingRef01Ent.List(ratingRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, ratingRef01ListOk := ratingRef01ListResult.([]any)
		if !ratingRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", ratingRef01ListResult)
		}
...
func ratingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FOOD_HYGIENE_RATING_TEST_RATING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FOOD_HYGIENE_RATING_TEST_RATING_ENTID"])
