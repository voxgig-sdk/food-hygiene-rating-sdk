...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FOODHYGIENERATING_TEST_LIVE") == "TRUE" ||
		os.Getenv("FOODHYGIENERATING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FOODHYGIENERATING_TEST_EXPLAIN"); explain != "" {
