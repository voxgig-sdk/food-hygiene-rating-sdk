...
The Lua SDK for the FoodHygieneRating API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Authority(nil):list(nil, nil)
...
```lua
local result, err = client:Authority(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:FoodHygieneRating(nil):load(
  { id = "test01" }, nil
)
