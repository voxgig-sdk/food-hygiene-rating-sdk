...
local RatingEntity = {}
RatingEntity.__index = RatingEntity


function RatingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RatingEntity)
  self._name = "rating"
  self._client = client
...
function RatingEntity:get_name()
  return self._name
end


function RatingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RatingEntity.new(self._client, opts)
end
...
function RatingEntity:mark_deleted()
  self._deleted = true
end


function RatingEntity:deleted()
  return true == self._deleted
end


function RatingEntity:data_set(args)
  if args ~= nil then
...
function RatingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RatingEntity:match_set(args)
  if args ~= nil then
...
function RatingEntity:match_get()
...
function RatingEntity:stream(action, args, callopts)
...
function RatingEntity:_run_op(ctx, post_done)
  local utility = self._utility
