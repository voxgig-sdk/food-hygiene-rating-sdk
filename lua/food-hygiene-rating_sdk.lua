...
require("food-hygiene-rating_types")
...
local FoodHygieneRatingSDK = {}
FoodHygieneRatingSDK.__index = FoodHygieneRatingSDK
...
FoodHygieneRatingSDK._make_feature = _make_feature


function FoodHygieneRatingSDK.new(options)
  local self = setmetatable({}, FoodHygieneRatingSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FoodHygieneRatingSDK:options_map()
...
function FoodHygieneRatingSDK:get_utility()
  return Utility.copy(self._utility)
end


function FoodHygieneRatingSDK:get_root_ctx()
  return self._rootctx
end


function FoodHygieneRatingSDK:prepare(fetchargs)
...
function FoodHygieneRatingSDK:direct(fetchargs)
...
function FoodHygieneRatingSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function FoodHygieneRatingSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "FoodHygieneRatingSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function FoodHygieneRatingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FoodHygieneRatingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FoodHygieneRatingSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FoodHygieneRatingSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FoodHygieneRatingSDK.new(sdkopts)
  sdk.mode = "test"
