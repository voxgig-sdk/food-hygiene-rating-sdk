...
describe("AuthorityDirect", function()
  it("should direct-list-authority", function()
    local setup = authority_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-authority", setup.live and "live" or "unit")
  ...
  it("should direct-load-authority", function()
    local setup = authority_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-authority", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function authority_direct_setup(mockres)
