...
describe("EstablishmentDirect", function()
  it("should direct-list-establishment", function()
    local setup = establishment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-establishment", setup.live and "live" or "unit")
  ...
  it("should direct-load-establishment", function()
    local setup = establishment_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-establishment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function establishment_direct_setup(mockres)
