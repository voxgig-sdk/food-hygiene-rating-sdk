...
// FoodHygieneRating SDK retry feature
...
class FoodHygieneRatingRetryFeature extends FoodHygieneRatingBaseFeature
{
    ...
    public function init(FoodHygieneRatingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FoodHygieneRatingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FoodHygieneRatingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FoodHygieneRatingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
