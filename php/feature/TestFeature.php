...
// FoodHygieneRating SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class FoodHygieneRatingTestFeature extends FoodHygieneRatingBaseFeature
{
    ...
    public function init(FoodHygieneRatingContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (FoodHygieneRatingContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = FoodHygieneRatingParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (FoodHygieneRatingContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(FoodHygieneRatingContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = FoodHygieneRatingParam::call($ctx, $k);
