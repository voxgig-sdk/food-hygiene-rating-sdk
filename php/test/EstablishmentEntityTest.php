...
class EstablishmentEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FoodHygieneRatingSDK::test(null, null);
        $ent = $testsdk->Establishment(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Establishment(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Establishment(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = establishment_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "establishment." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FOOD_HYGIENE_RATING_TEST_ESTABLISHMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $establishment_ref01_data = null;
        if (count($establishment_ref01_data_raw) > 0) {
...
function establishment_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/establishment/EstablishmentTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["establishment01", "establishment02", "establishment03"] as $k) {
    ...
    $entid_env_raw = getenv("FOOD_HYGIENE_RATING_TEST_ESTABLISHMENT_ENTID");
