...
//    only - a typed FoodHygieneRatingContext there would make every ctx
...
final class FoodHygieneRatingOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new FoodHygieneRatingContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new FoodHygieneRatingSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new FoodHygieneRatingResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new FoodHygieneRatingError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new FoodHygieneRatingResponse($clean['response']);
    ...
    public static function err_from_map($m): ?FoodHygieneRatingError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FoodHygieneRatingError(is_string($code) ? $code : '', $msg);
    }
