...
from foodhygienerating_sdk.foodhygienerating_types import (
    Establishment,
...
class EstablishmentEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EstablishmentEntity(self._client, opts)
    ...
    def data_get(self) -> Establishment:
    ...
    def match_get(self) -> Establishment:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Establishment:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Establishment().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Establishment]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Establishment().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
