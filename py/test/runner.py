# FoodHygieneRating SDK test runner
...
class FoodHygieneRatingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FoodHygieneRatingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FoodHygieneRatingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FoodHygieneRatingTestRunner.getenv("FOOD_HYGIENE_RATING_TEST_LIVE")
        override = FoodHygieneRatingTestRunner.getenv("FOOD_HYGIENE_RATING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FoodHygieneRatingTestRunner.getenv(key)
        ...
        explain = FoodHygieneRatingTestRunner.getenv("FOOD_HYGIENE_RATING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FoodHygieneRatingTestRunner._test_control is not None:
            return FoodHygieneRatingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FoodHygieneRatingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FoodHygieneRatingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FoodHygieneRatingTestRunner.load_test_control()
...
def load_env_local():
    FoodHygieneRatingTestRunner.load_env_local()


def env_override(m):
    return FoodHygieneRatingTestRunner.env_override(m)


def entity_data(v):
    return FoodHygieneRatingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FoodHygieneRatingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FoodHygieneRatingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FoodHygieneRatingTestRunner.load_test_control()


def live_client_options():
    return FoodHygieneRatingTestRunner.live_client_options()


def live_delay_ms():
    return FoodHygieneRatingTestRunner.live_delay_ms()
