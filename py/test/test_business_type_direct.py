...
class TestBusinessTypeDirect:

    def test_should_direct_list_business_type(self):
        setup = _business_type_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-business_type", "live" if setup["live"] else "unit")
        if _skip:
...
def _business_type_direct_setup(mockres):
