...
class TestBusinessTypeEntity:

    def test_should_create_instance(self):
        testsdk = FoodHygieneRatingSDK.test(None, None)
        ent = testsdk.BusinessType(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.BusinessType(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from foodhygienerating_sdk.config import make_config
        cfg = make_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = FoodHygieneRatingSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.BusinessType(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _business_type_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "business_type." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FOOD_HYGIENE_RATING_TEST_BUSINESS_TYPE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        business_type_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.business_type")))
        business_type_ref01_data = None
        if len(business_type_ref01_data_raw) > 0:
...
def _business_type_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/business_type/BusinessTypeTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FOOD_HYGIENE_RATING_TEST_BUSINESS_TYPE_ENTID"))
