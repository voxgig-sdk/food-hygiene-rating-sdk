...
class TestRatingDirect:

    def test_should_direct_list_rating(self):
        setup = _rating_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-rating", "live" if setup["live"] else "unit")
        if _skip:
...
def _rating_direct_setup(mockres):
