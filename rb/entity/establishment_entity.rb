...
class EstablishmentEntity
  ...
  def make
    opts = @_entopts.dup
    EstablishmentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FoodHygieneRatingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Establishment, Hash] the current Establishment data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FoodHygieneRatingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Establishment fields)
  def match_get
  ...
  #
  # @param reqmatch [EstablishmentLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Establishment.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Establishment, Hash] the loaded Establishment; raises FoodHygieneRatingError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FoodHygieneRatingHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [EstablishmentListMatch, Hash, nil] match filter (any subset of
  #   Establishment fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Establishment>, Array] the matching Establishment items; raises FoodHygieneRatingError on failure
