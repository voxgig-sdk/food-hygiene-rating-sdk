...
class BusinessTypeDirectTest < Minitest::Test
  def test_direct_list_business_type
    setup = business_type_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-business_type", setup[:live] ? "live" : "unit")
    if _should_skip
...
def business_type_direct_setup(mockres)
