...
class RatingDirectTest < Minitest::Test
  def test_direct_list_rating
    setup = rating_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-rating", setup[:live] ? "live" : "unit")
    if _should_skip
...
def rating_direct_setup(mockres)
