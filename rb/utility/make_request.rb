...
module FoodHygieneRatingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FoodHygieneRatingResponse.new({})
    ...
    if fetch_err
      ...
      response = FoodHygieneRatingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FoodHygieneRatingResponse.new(fetched)
    else
