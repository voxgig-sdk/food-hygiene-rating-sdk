...
(0, node_test_1.describe)('AuthorityDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.FoodHygieneRatingSDK({
    ...
    (0, node_test_1.test)('direct-load-authority', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-authority', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-authority', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-authority', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.FOOD_HYGIENE_RATING_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'FOOD_HYGIENE_RATING_TEST_AUTHORITY_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.FoodHygieneRatingSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['FOOD_HYGIENE_RATING_TEST_AUTHORITY_ENTID'];
    ...
    const client = new __1.FoodHygieneRatingSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=AuthorityDirect.test.js.map