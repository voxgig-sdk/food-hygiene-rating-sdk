...
//     (\`new FoodHygieneRatingSDK(...)\` / \`FoodHygieneRatingSDK.test(...)\` become
//     \`FoodHygieneRatingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FoodHygieneRatingSDK'
...
const TEST_SEED = {"entity":{"authority":{"test01":{"id":"test01"}},"business_type":{"test01":{"id":"test01"}},"establishment":{"test01":{"id":"test01"}},"rating":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FoodHygieneRatingSDK(...)\` and any
// \`FoodHygieneRatingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FoodHygieneRatingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FoodHygieneRatingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
