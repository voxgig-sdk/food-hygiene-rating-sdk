...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FOODHYGIENERATING_TEST_LIVE ||
    'TRUE' === process.env.FOODHYGIENERATING_TEST_OVERRIDE
  ) {
  ...
  m.FOODHYGIENERATING_TEST_EXPLAIN = process.env.FOODHYGIENERATING_TEST_EXPLAIN || m.FOODHYGIENERATING_TEST_EXPLAIN
